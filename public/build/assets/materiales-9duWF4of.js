const r=document.getElementById("contenedor-auditorias");let s,n=!1;document.addEventListener("click",o=>{const a=o.target.closest(".auditoria");if(a){const e=a.dataset.id;p(e),s=e,n=!0,document.getElementById("id-auditoria").innerHTML=`#${e}`,document.getElementById("auditoria-modal").classList.remove("hidden"),document.body.classList.add("overflow-hidden")}o.target.closest(".cerrar-modal-auditoria")&&(r.innerHTML="",n=!1,s=null)});async function p(o){const t=await(await fetch(`/admin/informes/laboratorios/laboratorio-normal/auditorias?id=${o}`)).json();m(t)}function m(o){if(o.length===0)r.innerHTML=`
        ...
        `,r.innerHTML=a}}setInterval(()=>{n&&p(s)},5e3);const l=document.getElementById("informacion-filtrada"),u=document.getElementById("buscador"),x=document.getElementById("filtro-tipo");async function i(){const a=await(await fetch(`/api/admin/informes/laboratorios/laboratorio-normal/buscador?idLab=${document.getElementById("id-lab").value}&texto=${u.value}&filtro=${x.value}`)).json();h(a)}let c;const f=300;u.addEventListener("input",()=>{clearTimeout(c),c=setTimeout(()=>{i()},f)});x.addEventListener("change",()=>{i()});setInterval(()=>{i()},5e3);function h(o){l.innerHTML="";let a="";o.forEach(t=>{const e=JSON.parse(t.info_usuario),d=JSON.parse(t.info_material),g=JSON.stringify(d).replace(/"/g,"&quot;");a+=`
            <tr class="hover:bg-gray-50/50 transition-colors group">
                <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                        <div class="min-w-0">
